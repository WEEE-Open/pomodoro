<script>
// https://iconoir.com/

export default {
	props: ['color', 'text']
}
</script>
<template>
	<svg width="80" height="80" stroke-width="1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path
			d="M14.2718 10.445L18 2M9.31612 10.6323L5 2M12.7615 10.0479L8.835 2M14.36 2L13.32 4.5M6 16C6 19.3137 8.68629 22 12 22C15.3137 22 18 19.3137 18 16C18 12.6863 15.3137 10 12 10C8.68629 10 6 12.6863 6 16Z"
			:stroke="color" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
		<text x="12" y="20" text-anchor="middle" stroke="none" :fill="color" font-size="12px">{{ text }}</text>
	</svg>
</template>